...
export default class Server {
	...
	private registerRoutes() {
		...
		this.express.put("/dataset/:id/:kind", Server.datasetAdder);
		this.express.post("/query", Server.resultsGetter);
		this.express.delete("/dataset/:id", Server.datasetRemover);
		this.express.get("/datasets", Server.datasetDisplayer);
	}

	private static async datasetAdder(req: Request, res: Response) {
		try {
			...
			let result = await Server.insightFacade.addDataset(id, buffer, datasetKind);
			res.status(200).json({result});
		} catch (error: any) {
			let status;
			switch (true) {
				case error instanceof InsightError:
					status = 400;
					break;
	...
	private static async resultsGetter(req: Request, res: Response) {
		try {
			let query = req.body;
	...
	private static async datasetDisplayer(req: Request, res: Response) {
		try {
	...
	private static async datasetRemover(req: Request, res: Response) {
		try {
